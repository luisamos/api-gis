...
from app.models import Sector, SectorHistorico
...
@sectores_bp.route("/sectores/subir_shapefile", methods=["POST"])
def subir_shapefile():
  payload, status = handle_shapefile_upload(request.files.get("file"))
  return jsonify(payload), status


@sectores_bp.route("/sectores/validar_shapefile", methods=["POST"])
def validar_shapefile():
  ...
  if not carpeta or not codigo_sector:
      return (
          jsonify(
              {
                  "estado": False,
                  "mensaje": "Los parámetros 'nombreCarpeta' y 'codigoSector' son obligatorios",
              }
          ),
          400,
      )
  ...
  if not sector_field:
      ...
      return (
          jsonify(
              {
                  "estado": False,
                  "mensaje": "El archivo Shapefile no contiene el campo de sector solicitado",
              }
          ),
          400,
      )
  ...
  field_map = {"cod_sector": sector_field}
...
@sectores_bp.route("/sectores/cargar_shapefile", methods=["POST"])
  ...
  try:
      with db.session.begin():
          if cod_sectores:
              existentes = (
                  db.session.execute(
                      select(Sector).where(Sector.cod_sector.in_(cod_sectores))
                  )
                  .scalars()
                  .all()
              )
              ...
              for sector in existentes:
                  historico = SectorHistorico.from_sector(
                      sector, id_usuario, fecha_operacion
                  )
