...
### 2.4.1 Extraer parte numérica de un campo de texto (opcional)

Cuando el valor de sector provenga de un texto o etiqueta CAD y se necesite
obtener únicamente la parte numérica del campo `nombre_campo`, usar la
Calculadora de Campos con la siguiente expresión:

```qgis
regexp_substr("nombre_campo", '[0-9]+')
```

Si el resultado debe almacenarse siempre con 2 dígitos, completar con cero a
la izquierda cuando corresponda:

```qgis
lpad(regexp_substr("nombre_campo", '[0-9]+'), 2, '0')
```
...
- Mantener siempre EPSG:32718\ o EPSG:32719\
