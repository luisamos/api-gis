...
$selectedDrive = $InstallDrive.ToUpper()
if ($selectedDrive.Length -eq 2 -and $selectedDrive[1] -eq ':') {
    # Normalizar para aceptar "d:" o "D:".
    $selectedDrive = "{0}:" -f [char]::ToUpper($selectedDrive[0])
}
else {
    throw "InstallDrive debe tener formato de unidad, por ejemplo: C:, D:, E: o F:."
}

if (-not $PSBoundParameters.ContainsKey('InstallRoot') -or [string]::IsNullOrWhiteSpace($InstallRoot)) {
    $InstallRoot = "$selectedDrive\apps\python\api-gis"
}

if (-not $PSBoundParameters.ContainsKey('VenvPath') -or [string]::IsNullOrWhiteSpace($VenvPath)) {
    $VenvPath = "$selectedDrive\apps\python\.venv"
}

$installRootDrive = [System.IO.Path]::GetPathRoot($InstallRoot)
$venvPathDrive = [System.IO.Path]::GetPathRoot($VenvPath)
if ([string]::IsNullOrWhiteSpace($installRootDrive) -or [string]::IsNullOrWhiteSpace($venvPathDrive)) {
    throw "InstallRoot y VenvPath deben ser rutas absolutas de Windows, por ejemplo: D:\apps\python\api-gis"
}

$installRootDrive = $installRootDrive.TrimEnd('\').ToUpper()
$venvPathDrive = $venvPathDrive.TrimEnd('\').ToUpper()
if ($installRootDrive -ne $venvPathDrive) {
    throw "InstallRoot y VenvPath deben usar la misma unidad. Recibido: InstallRoot=$installRootDrive y VenvPath=$venvPathDrive."
}

if ($PSBoundParameters.ContainsKey('InstallDrive') -and ($installRootDrive -ne $selectedDrive -or $venvPathDrive -ne $selectedDrive)) {
    throw "InstallDrive ($selectedDrive) no coincide con las rutas indicadas. Ajusta InstallRoot/VenvPath o usa la misma unidad."
}
...
function Ensure-GdalRuntime {
    ...
    $detectedProjPath = $null
    $projCandidates = @(
        $ProjLibPath,
        [Environment]::GetEnvironmentVariable('PROJ_LIB', 'Machine'),
        [Environment]::GetEnvironmentVariable('PROJ_LIB', 'Process'),
        [Environment]::GetEnvironmentVariable('PROJ_LIB', 'User')
    ) | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | Select-Object -Unique

    foreach ($candidate in $projCandidates) {
        if (Test-Path $candidate) {
            $detectedProjPath = $candidate
            break
        }
    }

    # Detectar instalaciones existentes de GDAL por si no usa la ruta por defecto.
    if (-not $detectedProjPath) {
        $gdalSearchRoots = @(
            "C:\Program Files\GDAL",
            "C:\Program Files\gdal",
            "C:\OSGeo4W",
            "C:\OSGeo4W64",
            "D:\Program Files\GDAL",
            "D:\Program Files\gdal",
            "D:\OSGeo4W",
            "D:\OSGeo4W64"
        ) | Select-Object -Unique

        foreach ($root in $gdalSearchRoots) {
            if (-not (Test-Path $root)) {
                continue
            }

            $projlibDir = Join-Path $root "projlib"
            if (Test-Path $projlibDir) {
                $detectedProjPath = $projlibDir
                break
            }

            $shareProjDir = Join-Path $root "share\proj"
            if (Test-Path $shareProjDir) {
                $detectedProjPath = $shareProjDir
                break
            }
        }
    }
    ...
    if (-not $detectedProjPath) {
        ...
        $detectedProjPath = $ProjLibPath
    }
    else {
        Write-Host "GDAL ya está instalado. Se usará PROJ_LIB en: $detectedProjPath"
    }

    [Environment]::SetEnvironmentVariable('PROJ_LIB', $detectedProjPath, 'Machine')
    [Environment]::SetEnvironmentVariable('PROJ_LIB', $detectedProjPath, 'Process')
    Write-Host "Variable de entorno PROJ_LIB registrada en: $detectedProjPath"
}
